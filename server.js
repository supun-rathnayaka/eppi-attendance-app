...
const bcrypt = require('bcryptjs'); // CRITICAL SECURITY FIX
...
app.post('/api/login', async (req, res) => {
    ...
    try {
        const user = await User.findOne({ username }); // Find user by username only
        ...
        // Return 401 for both user not found and password mismatch
...
// API 4: Leave Submission (Uses SendGrid)
app.post('/api/leave/submit', async (req, res) => {
    ...
    try {
        ...
        // 2. Send email notification via SendGrid API
        const msg = {
            to: process.env.ADMIN_EMAIL, // Recipient
            from: process.env.EMAIL_USER, // Your VERIFIED SendGrid Sender
        ...
        try {
            ...
            console.log('SendGrid email notification sent successfully.');
        } catch (emailError) {
            console.error("SendGrid email notification FAILED:", emailError);
